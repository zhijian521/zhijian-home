...
export function IconButton(props: IconButtonProps) {
    const { className, icon, id, label, isActive = false } = props;
    const rootClassName = clsx(styles.root, isActive && styles.active, className);

    if (props.asButton) {
        /*== 原生按钮用于触发本地操作，保留键盘交互语义 ==*/
        return (
            <button aria-label={label} className={rootClassName} id={id} onClick={props.onClick} title={label} type="button">
    ...
    return (
        <Link aria-current={isActive ? "page" : undefined} aria-label={label} className={rootClassName} href={props.href} id={id} title={label}>
            <Icon name={icon} size="1rem" />
        </Link>
    );
}
